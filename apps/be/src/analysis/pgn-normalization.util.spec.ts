...
describe('pgn-normalization.util', () => {
  it('normalizes CRLF line endings to LF', () => {
    expect(normalizePgn('[Event "Test"]\r\n\r\n1. e4 e5 1-0')).toBe(
      '[Event "Test"]\n\n1. e4 e5 1-0',
    );
  });

  it('collapses comment boundaries into parser-safe spacing', () => {
    expect(normalizePgn('1. e4 { [%eval 0.2] }{ Good } e5 1-0')).toBe(
      '1. e4 { [%eval 0.2] Good } e5 1-0',
    );
  });

  it('collapses repeated spaces and tabs', () => {
    expect(normalizePgn('1.\t e4   e5\t\t2. Nf3   Nc6 1-0')).toBe(
